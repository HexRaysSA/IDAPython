"""
summary: Programatically change the type and the name of a stack
    variable. For this sample we will use RtlImageNtHeader from
    ntdll.dll.
...
def main():
    if not ida_ida.inf_is_64bit() and ida_ida.inf_get_procname() != 'metapc':
    ...
    if func_ea != ida_idaapi.BADADDR:
        ...
        for func_item in func_items.code_items():
            ...
            if curr_insn.itype == ida_allins.NN_lea:
                if curr_insn.Op1.type == o_reg and curr_insn.Op1.is_reg(R_r9):
                    insn = curr_insn
                    continue

            if not curr_insn.itype == ida_allins.NN_call:
                continue
            ...
            if ida_funcs.get_func_name(curr_insn.Op1.addr) == callee_name:
                ...
                if ida_frame.get_func_frame(caller_frame, caller_func):
                    if insn == None:
                        print('Something weird happened')
                        break
                    else:
                        idx = caller_frame.get_stkvar(insn, insn.Op2, insn.Op2.addr)
